...
if ( class_exists( 'NectarThemeManager' ) ) $theme_skin = NectarThemeManager::$skin;
...
if ( strtolower( $color ) === 'accent-color' || strtolower( $color ) === 'extra-color-1' || strtolower( $color ) === 'extra-color-2' || strtolower( $color ) === 'extra-color-3' || isset( $custom_colors[strtolower( $color )] ) ) {
...
if ( strtolower( $color ) === 'accent-color tilt' || strtolower( $color ) === 'extra-color-1 tilt' || strtolower( $color ) === 'extra-color-2 tilt' || strtolower( $color ) === 'extra-color-3 tilt' ) {
...
if ( $button_style !== 'see-through-3d' ) {
    // Url
    if ( ! empty( $url ) ) {
        $href =  ' href="' . esc_attr( do_shortcode( $url ) ) . '"';
    } else {
        $href = '';
    }
	echo $button_open_tag_escaped . $href . ' ' . $color_or . $hover_color_override . $hover_text_color_override . $micromodal_trigger .'>';
...
// 3D Button Markup
	...
	echo '<div class="nectar-3d-transparent-button" ' .$button_ID_markup . $nofollow_attr . ' style="' .$margins. '" data-size="' . esc_attr( $size) . '">
        <a' . $href . ' ' . $target . ' role="button" class="' . esc_attr( $el_class ) . '"' . $micromodal_trigger . '><span class="hidden-text">' .wp_kses_post( $text) . '</span>
