...
if ( 'span' === $heading_tag ) $style .= ' class="nectar-button-type"';
...
// Micromodal
if ( $micromodal === 'true' ) {
    $micromodal_trigger = ' data-micromodal-open="modal"';
} else {
    $micromodal_trigger = '';
}
// Url
...
if ( $btn_style === 'material' ) {
    echo '<div class="nectar-cta ' . esc_attr( $class ) .esc_attr( $dynamic_el_styles ) . '" data-style="' . esc_attr( $btn_style ) . '" data-alignment="' . esc_attr( $alignment ) . '" data-display="' . esc_attr( $display ) . '" data-text-color="' . esc_attr( $text_color ) . '" ' . $style_markup . '>';
        ...
        echo '<span class="link_wrap" ' . $style . '><' . $anchor_tag . ' ' . $target . $nofollow_attr . ' class="link_text' . esc_attr( $link_text_classes ) . '" role="button"' . $href . $micromodal_trigger . '>' . wp_kses_post( $link_text ) . '<span class="circle" ' . $bg_style . '></span><span class="arrow"></span></' . $anchor_tag . '></span>';
...
// See through.
    ...
    echo '<div class="nectar-cta ' . esc_attr( $class) . esc_attr( $dynamic_el_styles ) . '" data-color="' . esc_attr( $button_color ) . '" data-using-bg="' . esc_attr( $using_bg_color ) . '" data-style="' . esc_attr( $btn_style ) . '" data-display="' . esc_attr( $display ) . '" data-alignment="' . esc_attr( $alignment ) . '" data-text-color="' . esc_attr( $text_color ) . '" ' . $style_markup . '>';
        ...
        echo '<span class="link_wrap" ' . $style_padding_markup . '><' . $anchor_tag . ' ' . $target . $nofollow_attr . ' class="link_text' . esc_attr( $link_text_classes ) . '" role="button"' . $href . $micromodal_trigger . '>' . wp_kses_post( $link_text ) . '<span class="arrow"></span></' . $anchor_tag . '></span>'; 
    echo '</' . esc_html( $heading_tag ) . '></div>';
} 
// Arrow Animation.
    ...
    echo '<div class="nectar-cta ' . esc_attr( $class) .esc_attr( $dynamic_el_styles ) . '" data-color="' . esc_attr( $button_color ) . '" data-using-bg="' . esc_attr( $using_bg_color ) . '" data-style="' . esc_attr( $btn_style ) . '" data-display="' . esc_attr( $display ) . '" data-alignment="' . esc_attr( $alignment ) . '" data-text-color="' . esc_attr( $text_color ) . '" ' . $style_markup . '>';
        ...
        echo '<span class="link_wrap" ' . $style_padding_markup . '><' . $anchor_tag . ' ' . $target . $nofollow_attr . ' class="link_text' . esc_attr( $link_text_classes ) . '" role="button"' . $href . $micromodal_trigger . '><span class="text">' . wp_kses_post( $link_text ) . '</span>'; 
        echo '<svg class="next-arrow" aria-hidden="true" width="20px" height="25px" viewBox="0 0 50 80" xml:space="preserve"><polyline stroke="#ffffff" stroke-width="9" fill="none" stroke-linecap="round" stroke-linejoin="round" points="0, 0 45, 40 0, 80"/></svg> ';
	...
	echo '<div class="nectar-cta ' . esc_attr( $class) .esc_attr( $dynamic_el_styles ) . '" data-color="' . esc_attr( $button_color ) . '" data-using-bg="' . esc_attr( $using_bg_color ) . '" data-style="' . esc_attr( $btn_style ) . '" data-display="' . esc_attr( $display ) . '" data-alignment="' . esc_attr( $alignment ) . '" data-text-color="' . esc_attr( $text_color ) . '" ' . $style_markup . '>';
        echo '<' . esc_html( $heading_tag ) . $style . '>';
        echo '<span class="link_wrap" ' . $style_padding_markup . '>' . $icon_output . '<' . $anchor_tag . ' ' . $target . $nofollow_attr . ' class="link_text' . esc_attr( $link_text_classes ) . '" role="button"' . $href . $micromodal_trigger . '><span class="text">' . wp_kses_post( $link_text ) . '</span>'; 
...
// Next section link.
    ...
    if ( $btn_type === 'down-arrow-bounce' ) {
        ...
        echo '<div class="nectar-next-section-wrap mouse-wheel' . esc_attr( $dynamic_el_styles ) . '" ' . $style_markup . ' data-align="' . esc_attr( $alignment ) . '" data-custom-color="' . esc_attr( $using_next_section_color ) . '"><a href="#" ' . $next_section_color_style . ' class="nectar-next-section skip-hash"><span class="screen-reader-text">' . esc_html__('Navigate to the next section','salient-core') . '</span><svg class="nectar-scroll-icon" viewBox="0 0 30 45" enable-background="new 0 0 30 45"><path class="nectar-scroll-icon-path" fill="none" stroke="' . esc_attr( $stroke_color ) . '" stroke-width="2" stroke-miterlimit="10" d="M15,1.118c12.352,0,13.967,12.88,13.967,12.88v18.76 c0,0-1.514,11.204-13.967,11.204S0.931,32.966,0.931,32.966V14.05C0.931,14.05,2.648,1.118,15,1.118z"></path></svg><span class="track-ball" ' . $next_section_track_ball_color_style . '></span></a></div>';
        ...
        echo '<div class="nectar-next-section-wrap minimal-arrow' . esc_attr( $dynamic_el_styles ) . '" ' . $style_markup . ' data-align="' . esc_attr( $alignment ) . '" data-custom-color="' . esc_attr( $using_next_section_color ) . '"><a href="#" class="nectar-next-section skip-hash"><span class="screen-reader-text">' . esc_html__('Navigate to the next section','salient-core') . '</span><svg class="next-arrow" width="40px" height="68px" viewBox="0 0 40 50" xml:space="preserve"><path stroke="' . esc_attr( $stroke_color ) . '" stroke-width="2" fill="none" d="M 20 0 L 20 51"></path><polyline stroke="' . esc_attr( $stroke_color ) . '" stroke-width="2" fill="none" points="12, 44 20, 52 28, 44"></polyline></svg></a></div>';
    }
}
// All others.
else {
    ...
    echo '<div class="nectar-cta ' . esc_attr( $class ) . esc_attr( $dynamic_el_styles ) . '" data-color="' . esc_attr( $button_color ) . '" data-using-bg="' . esc_attr( $using_bg_color ) . '" data-display="' . esc_attr( $display ) . '" data-style="' . esc_attr( $btn_style ) . '" data-alignment="' . esc_attr( $alignment ) . '" data-text-color="' . esc_attr( $text_color ) . '" ' . $style_markup . '>';
        ...
        echo '<span class="link_wrap" ' . $style_padding_markup . '>' . $icon_output . '<' . $anchor_tag . ' ' . $target . $nofollow_attr . ' class="link_text' . esc_attr( $link_text_classes ) . '" ' . $border_color_attr . ' role="button"' . $href . $micromodal_trigger . '>' . wp_kses_post( $link_text ) . '</' . $anchor_tag . '></span>'; 
    echo '</' . esc_html( $heading_tag ) . '></div>';
}